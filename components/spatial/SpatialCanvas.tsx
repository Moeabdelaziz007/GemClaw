...
const DraggableNode = ({ id, name, left, top, icon: Icon, onDragEnd, constraintsRef }: DragNodeProps) => {
  return (
    <motion.div
      drag
...
export const SpatialCanvas = () => {
  ...
  const [nodes, setNodes] = useState<{ [key: string]: Omit<DragNodeProps, 'onDragEnd' | 'constraintsRef'> }>({
  ...
  const handleDragEnd = (id: string, info: PanInfo) => {
    setNodes((prev) => {
      ...
      return {
        ...
        [id]: {
          ...node,
          left: Math.round(node.left + info.offset.x),
          top: Math.round(node.top + info.offset.y),
        },
      };
    });
  };
