...
export default function ForgeArchitect({ onComplete, onCancel }: ForgeArchitectProps) {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      role: 'architect',
      content: 'Welcome to the Aether Forge. I am the Architect. Together, we will materialize a new Sovereign Intelligence. What designation shall we bestow upon this entity?',
      field: 'name'
    }
  ]);
  ...
  const addMessage = (role: 'architect' | 'user', content: string, extra?: Partial<Message>) => {
    setMessages(prev => [...prev, { id: Math.random().toString(), role, content, ...extra }]);
  };
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    setInput('');
    addMessage('user', userValue);

    setIsTyping(true);
    await new Promise(r => setTimeout(r, 1000));
    setIsTyping(false);

    if (currentMsg.field === 'name') {
      setFormData(prev => ({ ...prev, name: userValue }));
      addMessage('architect', `A fine choice. ${userValue} shall be its name. Now, define its primary directive. What is its core purpose or role?`, { field: 'description' });
    } else if (currentMsg.field === 'description') {
